...
const game = {
  ...
  start() {
    ...
    while (+nextQuestionId !== -1) {
      ...
      const userAnswer = this.makeStep(nextQuestion);
      nextQuestionId = this.getQuestionId(userAnswer, nextQuestion);
    }    
  },


  getQuestionId(userAnswer, question) {
    let id;
    question.options.forEach((currentValue, index) => {
      if ((index+1)=== +userAnswer){
        id = currentValue.nextText;
      }
    })
    return id;
  },
